...
export default function Facilities({ facilities = [] }) {
  ...
  return <section className="facilities-section">
    ...
    <div className="facility-grid">{facilities.map((f) => { const I = icons[f.type] || LifeBuoy; return <div className="facility-card" key={f._id}><div className="facility-icon"><I size={20}/></div><div><strong>{f.name}</strong><span>{typeLabels[f.type] || f.type}</span><small>{f.distanceMeters} m {t('away')}</small></div></div>; })}</div>
  </section>;
}
